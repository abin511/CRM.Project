...
namespace CRM.BLL
{
    public abstract class BaseCrmManageServices<T> where T : class,new()
    {
        //在调用这个方法的时候必须给他赋值
        protected IBaseRepository<T> CurrentRepository { get; set; }
        //为了职责单一的原则，将获取线程内唯一实例的DbSession的逻辑放到工厂里面去了
        protected ICrmManageDbSession DbSession = DbSessionFactory.GetCrmManageDbSession();
        //基类的构造函数 构造函数里面调用了此设置当前仓储的抽象方法
        protected BaseCrmManageServices()
        {
